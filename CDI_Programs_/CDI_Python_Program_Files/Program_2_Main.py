...
Purpose:
    ...
    This is Program Version 2, which operates with 4 segment per cycles, along with a pre-run and post-run. 
...
How is this program different?
    This program differs as it is only a 4 segment cycle with the two new inter-segment 1 & 2
"""
...
def initialize_power_supply(excel_handler, power_supply_resource):
    ...
    pre_run_voltage = float(excel_handler.read_cell(4, 10))
    pre_run_current = float(excel_handler.read_cell(5, 10))
...
def read_experiment_parameters(excel_handler):
    ...
    All the inputs are hardcoded based on the excel set up this is for program 2 user inputs. Column 10
    ...
    parameters = {
        "reading_interval": float(excel_handler.read_cell(3, 10)),
        "pre_run_voltage": float(excel_handler.read_cell(4, 10)),
        "pre_run_current": float(excel_handler.read_cell(5, 10)),
        "pre_run_time": float(excel_handler.read_cell(6, 10)),
        "pre_run_flowrate": float(excel_handler.read_cell(7, 10)),

        "segment1_voltage": float(excel_handler.read_cell(9, 10)),
        "segment1_current": float(excel_handler.read_cell(10, 10)),
        "segment1_time": float(excel_handler.read_cell(11, 10)),
        "segment1_flowrate": float(excel_handler.read_cell(12, 10)),

        "inter_segment1_voltage": float(excel_handler.read_cell(14, 10)),
        "inter_segment1_current": float(excel_handler.read_cell(15, 10)),
        "inter_segment1_time": float(excel_handler.read_cell(16, 10)),
        "inter_segment1_flowrate": float(excel_handler.read_cell(17, 10)),
        
        
        "segment2_voltage": float(excel_handler.read_cell(19, 10)),
        "segment2_current": float(excel_handler.read_cell(20, 10)),
        "segment2_time": float(excel_handler.read_cell(21, 10)),
        "segment2_flowrate": float(excel_handler.read_cell(22, 10)),
        
        "inter_segment2_voltage": float(excel_handler.read_cell(24, 10)),
        "inter_segment2_current": float(excel_handler.read_cell(25, 10)),
        "inter_segment2_time": float(excel_handler.read_cell(26, 10)),
        "inter_segment2_flowrate": float(excel_handler.read_cell(27, 10)),

        "post_run_voltage": float(excel_handler.read_cell(29, 10)),
        "post_run_current": float(excel_handler.read_cell(30, 10)),
        "post_run_time": float(excel_handler.read_cell(31, 10)),
        "post_run_flowrate": float(excel_handler.read_cell(32, 10)),
...
def clear_all_unwanted(excel_handler):
    ...
    #excel_handler.delete_unused_macros(keep_modules=["Module7", "Module10", "Module9"])  #keeps only these modules 

    print("Clearing buttons")
    excel_handler.delete_unwanted_buttons("Inputs", keep_buttons=["Button 15", "Button 14", "Button 16"])  #Keeps only these buttons 

    print("Deleting sheets")
    sheets_to_delete = ["Program_1", "Program_3", "Program_4"] #Deletes these sheets
    for sheet_name in sheets_to_delete:
        excel_handler.delete_sheet(sheet_name)

def main():
    """
    Runs the CDI experiment from start to finish for Program 2
    ...
    try:
        ...
        excel_handler = ExcelHandler(workbook_path, "Program_2")
        ...
        segments = [
            ...
            ("Inter-Segment 1", 
             experiment_parameters["inter_segment1_voltage"], 
             experiment_parameters["inter_segment1_current"], 
             experiment_parameters["inter_segment1_flowrate"], 
             experiment_parameters["inter_segment1_time"]),
            ("Segment 2", 
            ...
            ("Inter-Segment 2", 
             experiment_parameters["inter_segment2_voltage"],
             experiment_parameters["inter_segment2_current"], 
             experiment_parameters["inter_segment2_flowrate"], 
             experiment_parameters["inter_segment2_time"]),
        ]
        ...
        while True:
           #prints out so users can see through the terminal
            ...
            for segment, voltage, current, flowrate, duration in segments:
                ...
                if stop_requested and segment == "Inter-Segment 2":
