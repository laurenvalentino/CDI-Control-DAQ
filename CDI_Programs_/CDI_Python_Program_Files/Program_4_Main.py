...
Purpose:
    ...
    This is Program Version 4, which operates with 2 segment per cycles, along with a pre-run and post-run. 
...
Data Collection:
    date, time, elapse time, voltage (V) 1, Current (mA) 1, voltage (V) 2, Current (mA)2, Conductivity (uS/cm), cycle, segment, and Flowrate

How is this program different?

    This program has the ability to have 2 applied voltages from the powersupply channel 1 and 2. These channels have been hardcoded in the program. If you 
    want to change dirrectly what channel you want either (1,2,3) you can change it in method initialize_power_supply line 119 and 120 replace "CH1" or "CH2" to your desired channel 
"""
...
def initialize_power_supply(excel_handler, power_supply_resource):
    ...
    based on values read from the Excel sheet (pre-run settings). This has 2 voltages for two different 
    channels in the power supply, Channel 1 & 2
    """
    print("Initializing Power Supply...")
    power_supply = psu.psu(power_supply_resource)
    
    CH1 = "CH1"  # Predefine channel 1, this will be used for pre-run and post-run
    CH2 = "CH2" 
    
    # Set initial voltage and current from pre-run settings in Excel
    pre_run_voltage = float(excel_handler.read_cell(4, 16))
    pre_run_current = float(excel_handler.read_cell(5, 16))
    pre_run_voltage2 = float(excel_handler.read_cell(4, 19))
    pre_run_current2 = float(excel_handler.read_cell(5, 19))
    
    power_supply.set_voltage_current_prg4(CH1, pre_run_voltage, pre_run_current)
    power_supply.set_voltage_current_prg4(CH2, pre_run_voltage2, pre_run_current2)
    
    print("Power Supply initialized with voltage:", pre_run_voltage, pre_run_voltage2, "and current:", pre_run_current, pre_run_current2 )
...
def read_experiment_parameters(excel_handler):
    ...
    All the inputs are hardcoded based on the excel set up this is for program 4 user inputs. Column 16 and 17 
    ...
    parameters = {
        "reading_interval": float(excel_handler.read_cell(3, 16)),
        "pre_run_voltage": float(excel_handler.read_cell(4, 16)),
        "pre_run_current": float(excel_handler.read_cell(5, 16)),
        "pre_run_time": float(excel_handler.read_cell(6, 16)),
        "pre_run_flowrate": float(excel_handler.read_cell(7, 16)),
        
        
        "pre_run_voltage2": float(excel_handler.read_cell(4, 19)),
        "pre_run_current2": float(excel_handler.read_cell(5, 19)),

        "segment1_voltage": float(excel_handler.read_cell(9, 16)),
        "segment1_current": float(excel_handler.read_cell(10, 16)),
        "segment1_time": float(excel_handler.read_cell(11, 16)),
        "segment1_flowrate": float(excel_handler.read_cell(12, 16)),
        
        
        "segment1_voltage2": float(excel_handler.read_cell(9, 19)),
        "segment1_current2": float(excel_handler.read_cell(10, 19)),

        "segment2_voltage": float(excel_handler.read_cell(19, 16)),
        "segment2_current": float(excel_handler.read_cell(20, 16)),
        "segment2_time": float(excel_handler.read_cell(21, 16)),
        "segment2_flowrate": float(excel_handler.read_cell(22, 16)),
        
        "segment2_voltage2": float(excel_handler.read_cell(19, 19)),
        "segment2_current2": float(excel_handler.read_cell(20, 19)),

        "post_run_voltage": float(excel_handler.read_cell(29, 16)),
        "post_run_current": float(excel_handler.read_cell(30, 16)),
        "post_run_time": float(excel_handler.read_cell(31,16)),
        "post_run_flowrate": float(excel_handler.read_cell(32, 16)),
        
        "post_run_voltage2": float(excel_handler.read_cell(29, 19)),
        "post_run_current2": float(excel_handler.read_cell(30, 19)),
...
def collect_and_log_data(excel_handler, power_supply, probe_device, overall_start_time, segment_name, flowrate, reading_interval, last_logged_time, cycle_count, k_value):
    ...
    if elapsed_since_last_log >= reading_interval:
        ...
        voltage = power_supply.measure_voltage_prg4("CH1")
        current = power_supply.measure_current_prg4("CH1")
        voltage2 = power_supply.measure_voltage_prg4("CH2")
        current2 = power_supply.measure_current_prg4("CH2")
        ...
        excel_handler.write_data_program4(elapsed_time, voltage, current, voltage2, current2, probe_reading, segment_name, flowrate, cycle_count)
...
def clear_all_unwanted(excel_handler):
    ...
    #excel_handler.delete_unused_macros(keep_modules=["Module7", "Module3", "Module9"]) #keeps only these modules 

    print("Clearing buttons")
    excel_handler.delete_unwanted_buttons("Inputs", keep_buttons=["Button 27", "Button 26", "Button 28"]) #Keeps only these buttons 

    print("Deleting sheets")
    sheets_to_delete = ["Program_1", "Program_2", "Program_3"] #Deletes these sheets
    for sheet_name in sheets_to_delete:
        excel_handler.delete_sheet(sheet_name)

def main():
    """
    Runs the CDI experiment from start to finish for Program 4
    ...
    try:
        ...
        CH1 = "CH1"  # Predefine channel 1
        CH2 = "CH2" 
        
        # Initialize Excel Handler with provided workbook path
        excel_handler = ExcelHandler(workbook_path, "Program_4")
        ...
        pre_run_voltage_2 = experiment_parameters["pre_run_voltage2"]
        pre_run_current_2 = experiment_parameters["pre_run_current2"]
        ...
        power_supply.set_voltage_current_prg4(CH1, pre_run_voltage, pre_run_current)
        power_supply.set_voltage_current_prg4(CH2,pre_run_voltage_2,pre_run_current_2)
        ...
        segments = [
            ("Segment 1", 
             ...
             experiment_parameters["segment1_voltage2"], 
             experiment_parameters["segment1_current2"], 
            ...
            ("Segment 2", 
             ...
             experiment_parameters["segment2_voltage2"], 
             experiment_parameters["segment2_current2"], 
        ...
        while True:
           #prints out so users can see through the terminal
            ...
            for segment, voltage1, current1, voltage2, current2, flowrate, duration in segments:
                print(f"Starting {segment}...")
                segment_start_time = time.perf_counter()
                power_supply.set_voltage_current_prg4(CH1, voltage1, current1)
                power_supply.set_voltage_current_prg4(CH2, voltage2, current2)
        ...
        post_run_voltage2 = experiment_parameters["post_run_voltage2"]
        post_run_current2 = experiment_parameters["post_run_current2"]
        
        post_run_flowrate = experiment_parameters["post_run_flowrate"]
        post_run_time = experiment_parameters["post_run_time"]
        
        power_supply.set_voltage_current_prg4(CH1,post_run_voltage, post_run_current)
        power_supply.set_voltage_current_prg4(CH2, post_run_voltage2, post_run_current2)
